...
const InputLabel = ({label, name, type, color, value, maxWidth} : FormInputInt) => {
    let label_style_string = `text-${color??"orange"}-400 rounded-lg   text-center`
    ...
    return (
        <div className="input-label p-5" style={{maxWidth}}>
            ...
            <input className={input_style_string} type={type} name={name} value={inputValue} onChange={handleChange} />
        </div>
    );
}
